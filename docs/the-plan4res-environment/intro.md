...
-   Run the install script: *./plan4res_install.sh* with needed options
    ...
    HiGHS ; -I \$installer if \$SOLVER is CPLEX or GUROBI; -L \$LICENCE
    if \$SOLVER is GUROBI; ....)

-   If you wish to run plan4res from a different directory, e.g
    P4R_DIR_LOCAL, move the script user_init_plan4res.sh in
    P4R_DIR_LOCAL and run it as follows: *./user_init_plan4res.sh -D
    P4R_DIR -S \$SOLVER*
